...
test("workflow run lifecycle transitions", () => {
    ...
    assert.throws(() => run.cancel("already done"), (error) => error instanceof DomainError);
});
test("workflow run cannot complete before it is running", () => {
    ...
    assert.throws(() => run.complete(), (error) => error instanceof DomainError);
});
//# sourceMappingURL=workflow-run.test.js.map