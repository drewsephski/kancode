...
test("request to completion end-to-end", async () => {
  ...
  const completeSessionHandler = new CompleteExecutionSessionHandler(
    executionSessionRepo,
    assignmentRepo,
    taskRepo,
    workflowRunRepo,
  ...
  const { workflowId, workflowRunId } = await planHandler.handle(
  ...
  await completeSessionHandler.handle(
  ...
  const eventTypes = eventPublisher.publishedFlat.map((e: { type: string }) => e.type);
  ...
  for (const expected of expectedEvents) {
    assert.ok(
      ...
      `expected event "${expected}" was not emitted (got [${eventTypes.join(", ")}])`,
    );
  }
