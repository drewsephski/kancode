...
test("process manager drives full request-to-completion flow", async () => {
  ...
  async function drainOutbox(times = 10): Promise<void> {
  ...
  const { requestId } = await submitHandler.handle(
    { type: "request.submit", payload: { requestText: "Process-managed request" } },
    {
  ...
  await completeHandler.handle(
    { type: "execution_session.complete", payload: { executionSessionId: sessionId, outputSummary: "Completed by test" } },
    { workspaceId: "workspace-1", actorId: "runtime", correlationId: "corr-complete", idempotencyKey: "runtime-complete-1" },
  );
...
test("process manager handles event out of order gracefully", async () => {
  ...
  const repo = () => ({ getById: async () => null, save: async () => {} }) as any;

  const pm = new WorkflowRunProcessManager(
    processRepo, new FakeTransitionAuditLog(), new FakeMetricsRecorder(), idGenerator, clock,
    {
  ...
  await pm.handle(
    { type: "execution_session.completed", occurredAt: new Date(), payload: { id: "es-1" }, version: 1 },
    ctx,
  );
  await pm.handle(
    { type: "request.completed", occurredAt: new Date(), payload: { id: "req-1" }, version: 1 },
    ctx,
  );
