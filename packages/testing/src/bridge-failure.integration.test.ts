...
class FakeBridgePublisher implements BridgeCommandPublisher {
  readonly assignments: Array<Record<string, string>> = [];
  readonly executionStarts: Array<Record<string, string>> = [];

  async publishAssignment(input: Record<string, string>): Promise<void> {
    this.assignments.push(input);
  }
  async publishExecutionStart(input: Record<string, string>): Promise<void> {
...
class FakeSender {
  ...
  send: SendEventFn = async (p: EventBridgePayload) => { this.sent.push(p); };
}
...
// ---------------------------------------------------------------------------
async function setupThroughAssignment(
  ctx: Context,
): Promise<{ processId: string; assignmentId: string; executionSessionId: string }> {
  const sender = new FakeSender();
  const router = new TriggerEventRouter(ctx.processManager);
  const dispatcher: EventDispatcher = new TriggerDispatcher(ctx.outboxRepo, ctx.dlq, ctx.idGenerator, ctx.clock, { sendEvent: sender.send, maxRetries: 1 }, ctx.metrics);
  ...
  await ctx.submitHandler.handle(
    { type: "request.submit", payload: { requestText: "Failure test" } },
    { workspaceId: "workspace-1", actorId: "user-1", correlationId: "fail-corr", idempotencyKey: "fail-idem" },
  );
  ...
  const { executionSessionId } = await ctx.createSessionHandler.handle(
    { type: "execution_session.create", payload: { assignmentId: assignments[0]!.id, runtimeName: "Bridge" } },
    { workspaceId: "workspace-1", actorId: "bridge", correlationId: `fail-session-${processId}`, idempotencyKey: `fail-ses-${assignments[0]!.id}` },
  );
...
test("Scenario A: bridge failure transitions process and workflow run to failed", async () => {
  ...
  await router.routeEvent(
    { type: "execution_session.failed", occurredAt: new Date(), payload: { id: executionSessionId, errorMessage: "Bridge runtime error" }, version: 1 },
    { workspaceId: "workspace-1", actorId: "bridge", correlationId: processId, idempotencyKey: `fail-ev-${executionSessionId}` },
  );
...
test("Scenario B: duplicate execution_session.completed is safely ignored", async () => {
  ...
  await ctx.completeHandler.handle(
    { type: "execution_session.complete", payload: { executionSessionId, outputSummary: "Done" } },
    { workspaceId: "workspace-1", actorId: "bridge", correlationId: `dup-c-${processId}`, idempotencyKey: `dup-cc-${executionSessionId}` },
  );
  ...
  await router.routeEvent(
    { type: "execution_session.completed", occurredAt: new Date(), payload: { id: executionSessionId }, version: 1 },
    { workspaceId: "workspace-1", actorId: "bridge", correlationId: processId, idempotencyKey: `dup-ev-${executionSessionId}` },
  );
...
test("Scenario C: out-of-order execution_session.started is replay-safe", async () => {
  ...
  await router.routeEvent(
    { type: "execution_session.started", occurredAt: new Date(), payload: { id: "session-ooo", workspaceId: "workspace-1" }, version: 1 },
    { workspaceId: "workspace-1", actorId: "bridge", correlationId: processId, idempotencyKey: "ooo-started" },
  );
...
test("Scenario D: bridge reconnect replay is idempotent", async () => {
  ...
  // Complete the session
  await ctx.completeHandler.handle(
    { type: "execution_session.complete", payload: { executionSessionId, outputSummary: "Reconnect test" } },
    { workspaceId: "workspace-1", actorId: "bridge", correlationId: `recon-c-${processId}`, idempotencyKey: `recon-cc-${executionSessionId}` },
  );
  ...
  await router.routeEvent(
    { type: "execution_session.completed", occurredAt: new Date(), payload: { id: executionSessionId, outputSummary: "Reconnect replay" }, version: 1 },
    { workspaceId: "workspace-1", actorId: "bridge", correlationId: processId, idempotencyKey: `recon-r-${executionSessionId}` },
  );
