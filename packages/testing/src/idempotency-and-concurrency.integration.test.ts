...
function createContextOverrides(overrides?: Partial<{
  workspaceId: string;
  actorId: string;
...
async function runFullFlow(infra: ReturnType<typeof createInfrastructure>, context: ReturnType<typeof createContextOverrides>) {
  // Step 1: Submit Request
  const { requestId } = await infra.submitHandler.handle(
    { type: "request.submit" as const, payload: { requestText: "Build Stripe subscriptions" } },
    context,
  );

  // Step 2: Plan Workflow Run
  const { workflowRunId } = await infra.planHandler.handle(
    { type: "workflow.plan" as const, payload: { requestId } },
    context,
  );

  // Get the task for this specific workflow run
  const tasks = [...infra.taskRepo.store.values()].filter(
    (t) => t.workflowRunId === workflowRunId,
  );
  const taskId = tasks[0]!.id;

  // Step 3: Assign Task
  const { assignmentId } = await infra.assignHandler.handle(
    { type: "task.assign" as const, payload: { taskId } },
    context,
  );

  // Step 4: Create Execution Session
  const { executionSessionId } = await infra.createSessionHandler.handle(
    { type: "execution_session.create" as const, payload: { assignmentId, runtimeName: "Claude Code" } },
    context,
  );

  // Step 5: Complete Execution Session
  await infra.completeHandler.handle(
...
test("idempotency: AssignTaskCommand twice with same idempotency key", async () => {
  ...
  const { requestId } = await infra.submitHandler.handle(
    { type: "request.submit" as const, payload: { requestText: "Build" } },
    context,
  );
  await infra.planHandler.handle(
    { type: "workflow.plan" as const, payload: { requestId } },
    context,
  );
...
test("idempotency: CompleteExecutionSessionCommand twice with same idempotency key", async () => {
  ...
  const completionEvents = infra.eventPublisher.publishedFlat.filter(
    (e) => e.type === "execution_session.completed",
  );
...
test("idempotency: different idempotency keys create separate aggregates", async () => {
  ...
  await handler.handle(
    { type: "request.submit" as const, payload: { requestText: "First request" } },
    createContextOverrides({ idempotencyKey: "idem-first" }),
  );

  // Even with same payload but different key, should create separate aggregates
  await handler.handle(
    { type: "request.submit" as const, payload: { requestText: "First request" } },
    createContextOverrides({ idempotencyKey: "idem-second" }),
  );
...
test("concurrency: second assignment attempt against the same task throws", async () => {
  ...
  await infra.assignHandler.handle(
    { type: "task.assign" as const, payload: { taskId } },
    context,
  );
...
test("concurrency: duplicate complete execution session throws", async () => {
  ...
  await assert.rejects(
    () =>
      infra.completeHandler.handle(
        { type: "execution_session.complete" as const, payload: { executionSessionId: ids.executionSessionId, outputSummary: "again" } },
        createContextOverrides({ idempotencyKey: "second-complete" }),
      ),
    (err: unknown) => {
      assert.ok(err instanceof Error);
      return true;
...
test("version: every state transition increments version exactly once", async () => {
  ...
  const requestEvents = infra.eventPublisher.publishedFlat.filter(
    (e) => e.type.startsWith("workflow."),
  );
...
test("version: two separate submissions each track their own version", async () => {
  ...
  const r1 = await handler.handle(
    { type: "request.submit" as const, payload: { requestText: "Req A" } },
    createContextOverrides({ idempotencyKey: "a" }),
  );
  const r2 = await handler.handle(
    { type: "request.submit" as const, payload: { requestText: "Req B" } },
    createContextOverrides({ idempotencyKey: "b" }),
  );
...
test("event replay: all emitted events can be serialized and replayed deterministically", async () => {
  ...
  const deserialized = JSON.parse(serialized) as DomainEvent[];
...
test("event replay: duplicate event handling is safe", async () => {
  ...
  const requestIds = completedRequests.map((e) => (e.payload as { requestId: string }).requestId);
