...
function t(state, event, payload = {}) {
...
test("state machine: random event ordering converges to same final state", () => {
    ...
    for (let seed = 0; seed < 10; seed++) {
        ...
        assert.ok(["pending", "planning", "awaiting_assignment", "awaiting_execution", "executing", "completing", "completed"].includes(state), `seed ${seed}: invalid terminal state ${state}`);
    }
});
// ── Edge cases ──────────────────────────────────────────────────────────
test("state machine: completed is terminal and ignores all events", () => {
    for (const event of ["request.submitted", "workflow_run.created", "task.created", "task.assigned"]) {
