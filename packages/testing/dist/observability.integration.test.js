...
test("metrics are recorded for process manager transitions", async () => {
    ...
    const { requestId } = await (new SubmitRequestHandler(workspaceRepo, requestRepo, unitOfWork, idGenerator, clock, eventPublisher)).handle({ type: "request.submit", payload: { requestText: "Metrics test" } }, { workspaceId: "workspace-1", actorId: "user-1", correlationId: "corr-submit", idempotencyKey: "s1" });
    await drain();
    const sessions = [...executionSessionRepo.store.values()];
    if (sessions.length > 0) {
        await completeHandler.handle({ type: "execution_session.complete", payload: { executionSessionId: sessions[0].id, outputSummary: "done" } }, { workspaceId: "workspace-1", actorId: "runtime", correlationId: "corr-complete", idempotencyKey: "c1" });
    }
    await drain();
