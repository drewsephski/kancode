...
import { FakeClock, FakeIdGenerator, FakeRequestRepository, FakeWorkspaceRepository, FakeWorkflowRepository, FakeWorkflowRunRepository, FakeTaskRepository, FakeAssignmentRepository, FakeExecutionSessionRepository, FakeDomainEventStore, FakeOutboxRepository, InMemorySubscriberRegistry, } from "@kancode/testing";
import { SubmitRequestHandler, PlanWorkflowRunHandler, OutboxBackedEventPublisher, } from "@kancode/application";
...
test("event-driven flow: request.submitted triggers workflow planning via outbox", async () => {
    ...
    registry.register("request.submitted", async (event) => {
        ...
        await planHandler.handle({ type: "workflow.plan", payload: { requestId: payload.id } }, {
    ...
    const { requestId } = await submitHandler.handle({ type: "request.submit", payload: { requestText: "Event-driven workflow" } }, {
        workspaceId: "workspace-1",
        actorId: "user-1",
...
//# sourceMappingURL=event-driven-flow.integration.test.js.map