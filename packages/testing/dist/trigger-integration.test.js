...
function buildTestContext() {
    ...
    const processManager = new WorkflowRunProcessManager(processRepo, auditLog, metrics, idGenerator, clock, planHandler, assignHandler, createSessionHandler);
    return {
...
async function routeCapturedEvents(payloads, router) {
...
function resetRoutingProcessId() {
...
test("Test A: full trigger flow from submit request through process manager", async () => {
    ...
    const { requestId } = await ctx.submitHandler.handle({ type: "request.submit", payload: { requestText: "Trigger integration test" } }, { workspaceId: "workspace-1", actorId: "user-1", correlationId: "corr-a", idempotencyKey: "a1" });
    ...
    assert.ok(tasks[0].status === "open" || tasks[0].status === "in_progress", `expected task status to be "open" or "in_progress", got "${tasks[0].status}"`);
});
// ===========================================================================
...
test("Test B: task.created event triggers assignment via process manager", async () => {
    ...
    await ctx.submitHandler.handle({ type: "request.submit", payload: { requestText: "Assignment test" } }, { workspaceId: "workspace-1", actorId: "user-1", correlationId: "corr-b", idempotencyKey: "b1" });
...
test("Test C: completed execution routes through to request completion", async () => {
    ...
    await ctx.submitHandler.handle({ type: "request.submit", payload: { requestText: "Completion test" } }, { workspaceId: "workspace-1", actorId: "user-1", correlationId: "corr-c", idempotencyKey: "c1" });
    ...
    await ctx.completeHandler.handle({ type: "execution_session.complete", payload: { executionSessionId: session.id, outputSummary: "done" } }, { workspaceId: "workspace-1", actorId: "runtime", correlationId: "corr-c-complete", idempotencyKey: "c2" });
    ...
    const updatedRequest = await ctx.requestRepo.getById([...ctx.requestRepo.store.keys()][0]);
...
test("Test D: dispatcher failure moves events to dead-letter queue", async () => {
    ...
    const dispatcher = new TriggerDispatcher(outboxRepo, dlq, idGenerator, clock, { sendEvent: failingSender, maxRetries: 1 }, metrics);
    // Enqueue an event
    ...
    assert.ok(metrics.increments.some((m) => m.metric === "trigger.events.failed"), "trigger.events.failed metric should be recorded");
});
// ===========================================================================
...
test("trigger event metadata preserves all required fields", async () => {
    ...
    const fakeSender = new FakeTriggerSender();
    const metrics = new FakeMetricsRecorder();
    const dispatcher = new TriggerDispatcher(outboxRepo, dlq, idGenerator, clock, { sendEvent: fakeSender.send, maxRetries: 1 }, metrics);
    const testEvent = {
    ...
    // Verify metrics
    assert.ok(metrics.increments.some((m) => m.metric === "trigger.events.sent"), "trigger.events.sent metric should be recorded");
});
//# sourceMappingURL=trigger-integration.test.js.map