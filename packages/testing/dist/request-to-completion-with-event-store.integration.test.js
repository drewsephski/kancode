...
test("request to completion with event store and outbox", async () => {
    ...
    const eventStore = {
        ...
        async getByAggregateId(aggregateId) {
            return storedEvents.filter((e) => {
                ...
                return (payload.workspaceId === aggregateId ||
    ...
    const submitHandler = new SubmitRequestHandler(workspaceRepo, requestRepo, unitOfWork, idGenerator, clock, outboxEventPublisher);
    const planHandler = new PlanWorkflowRunHandler(requestRepo, workflowRepo, workflowRunRepo, taskRepo, unitOfWork, planner, outboxEventPublisher);
    const assignHandler = new AssignTaskHandler(taskRepo, assignmentRepo, unitOfWork, idGenerator, outboxEventPublisher);
    const createSessionHandler = new CreateExecutionSessionHandler(assignmentRepo, executionSessionRepo, unitOfWork, idGenerator, outboxEventPublisher);
    const completeHandler = new CompleteExecutionSessionHandler(executionSessionRepo, assignmentRepo, taskRepo, workflowRunRepo, requestRepo, unitOfWork, outboxEventPublisher);
    const context = {
        workspaceId: "workspace-1",
        actorId: "user-1",
    ...
    const { workflowRunId } = await planHandler.handle({ type: "workflow.plan", payload: { requestId } }, context);
    const tasks = [...taskRepo.store.values()].filter((t) => t.workflowRunId === workflowRunId);
    const { assignmentId } = await assignHandler.handle({ type: "task.assign", payload: { taskId: tasks[0].id } }, context);
    ...
    await completeHandler.handle({ type: "execution_session.complete", payload: { executionSessionId, outputSummary: "All done" } }, context);
    // ── Assertions ─────────────────────────────────────────────────────────
    ...
    assert.deepEqual(replayedEvents.map((e) => ({ type: e.type, version: e.version })), allEvents.map((e) => ({ type: e.type, version: e.version })));
    // 6. Version monotonicity is preserved in the event store
    const versionByPrefix = new Map();
...
//# sourceMappingURL=request-to-completion-with-event-store.integration.test.js.map