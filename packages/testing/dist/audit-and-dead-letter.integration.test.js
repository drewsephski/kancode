...
test("audit trail records every process state transition", async () => {
    ...
    async function drain(times = 10) { for (let i = 0; i < times; i++) {
        const p = await outboxRepo.getPending();
        if (p.length === 0)
            break;
        await dispatcher.dispatchPending();
    } }
    const { requestId } = await submitHandler.handle({ type: "request.submit", payload: { requestText: "Audit test" } }, { workspaceId: "workspace-1", actorId: "user-1", correlationId: "corr-submit", idempotencyKey: "s1" });
    await drain();
    const pendingSessions = [...executionSessionRepo.store.values()];
    if (pendingSessions.length > 0) {
        await completeHandler.handle({ type: "execution_session.complete", payload: { executionSessionId: pendingSessions[0].id, outputSummary: "done" } }, { workspaceId: "workspace-1", actorId: "runtime", correlationId: "corr-complete", idempotencyKey: "c1" });
    }
    await drain();
    // Verify audit trail
...
test("audit trail for out-of-order events is sparse but correct", async () => {
    ...
    const pm = new WorkflowRunProcessManager(processRepo, auditLog, new FakeMetricsRecorder(), idGenerator, clock, nullHandler, nullAssign, nullCreate);
...
//# sourceMappingURL=audit-and-dead-letter.integration.test.js.map