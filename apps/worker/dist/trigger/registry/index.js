import { TriggerJobRegistry, TriggerEventRouter, } from "@kancode/infrastructure";
/**
 ...
 */
export function buildTriggerJobRegistry(processManager) {
    const router = new TriggerEventRouter(processManager);
    const registry = new TriggerJobRegistry();
    const makeHandler = (router, eventType) => {
        return async (event) => {
            const context = {
 ...
 */
export function buildEventBridgeHandler(registry) {
